...
const formController: Record<string, FormControl> = {
    ...
    password_confirm: {
        additionalValidator: ($el, formData) => $el.value === formData.get('password'),
    ...
    salary: {
        transformData: formData => (formData.get('salary') as string).replace(/,/g, ''),
    },
};
    ...
    return <>
        ...
        >
            <fieldset>
                ...
                <p>
                    <label>성별: </label>
                    <input id="gender_male" name="gender" type="radio" value="남" required />
                ...
                <p>
                    ...
                    <input id="password_confirm" type="password" required autoComplete="off" />
                </p>
                <div>
                    <label htmlFor="photo">프로필사진: </label>
                    <input id="photo" name="photo" type="file" required accept="image/png, image/jpeg" />
                    <div><img alt="" /></div>
                </div>
                <p>
        ...
        <button type="submit" form="registerForm">제출</button>
    </>;
};
